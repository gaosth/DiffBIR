"""
Stage2训练脚本 - 褪色图片修复版本
只训练ControlNet，用于修复褪色的古画

与原始train_stage2.py的主要区别：
1. 不使用SwinIR，直接用褪色图片作为ControlNet的条件
2. 使用自定义的FadingRestorationDataset
3. 条件c_img直接来自褪色图片的VAE编码
"""
...
def main(args) -> None:
    ...
    # 注意：不再加载SwinIR，因为我们直接使用褪色图片作为条件
    if accelerator.is_main_process:
        print("Fading restoration mode: Using degraded images directly as condition")

    diffusion: Diffusion = instantiate_from_config(cfg.model.diffusion)

    # Setup optimizer - 只优化ControlNet参数:
